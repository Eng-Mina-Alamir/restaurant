...
void main() {
  group('ReservationRepository Unit Tests', () {
    ...
    test(
      'getReservations returns seeded list sorted by reservationTime',
      () async {
        final result = await repository.getReservations();
        expect(result.isRight, isTrue);
        expect(
          (result as Right<Failure, List<ReservationEntity>>).value.length,
          3,
        );
      },
    );
    ...
    test('createReservation adds a new reservation', () async {
      ...
      expect(
        (all as Right<Failure, List<ReservationEntity>>).value.any(
          (r) => r.id == 'res-999',
        ),
        isTrue,
      );
    ...
    test(
      'updateStatus updates reservation status or returns failure if not found',
      () async {
        ...
        expect(
          (updateResult as Right<Failure, ReservationEntity>).value.status,
          ReservationStatus.seated,
        );

        final notFoundResult = await repository.updateStatus(
          'res-404',
    ...
    test('cancelReservation sets status to cancelled', () async {
      ...
      final all = await repository.getReservations();
      final cancelled = (all as Right<Failure, List<ReservationEntity>>).value
          .firstWhere((r) => r.id == 'res-102');
