...
void main() {
  group('Loyalty Tier & Calculation Precision Tests', () {
    test('LoyaltyTier.fromPoints correctly assigns tiers at exact threshold boundaries', () {
      // Bronze: 0 .. 499
    ...
    test('Earning points applies tier multiplier and updates current & lifetime points', () async {
    ...
    test('Redeeming rewards fails with Failure if insufficient points', () async {
      final repo = InMemoryLoyaltyRepository();
      ...
      result.when(
        ...
        onRight: (_) => fail('Should not allow redeeming with insufficient points'),
      );
    });

    test('Redeeming reward deducts points from currentPoints but preserves lifetimePoints', () async {
      final repo = InMemoryLoyaltyRepository();
      ...
      result.when(
        ...
        onRight: (updated) {
          ...
          expect(updated.lifetimePoints, startLifetime); // Lifetime points must NOT decrease!
        },
      );
    });
  });
