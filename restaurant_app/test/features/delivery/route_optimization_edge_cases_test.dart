...
void main() {
  group('RouteOptimizationService Edge Cases Tests', () {
    ...
    test('handles multiple stops with identical coordinates (same building/tower)', () {
      const locationTower = LatLng(24.7200, 46.6800);
      const stops = [
        DeliveryStop(id: 'S-TOWER-1', name: 'شقة 401', location: locationTower),
        DeliveryStop(id: 'S-TOWER-2', name: 'شقة 802', location: locationTower),
        DeliveryStop(id: 'S-TOWER-3', name: 'شقة 1205', location: locationTower),
      ];
    ...
    test('safely falls back when negative or zero speed/per-stop values are passed', () {
      const stop = DeliveryStop(
        id: 'S-1',
        name: 'عميل',
        location: LatLng(24.7300, 46.6900),
    ...
    test('isWithinDeliveryRadius enforces geographic service boundary', () {
      ...
      expect(
        service.isWithinDeliveryRadius(restaurant, closeCustomer, maxRadiusKm: 25.0),
        isTrue,
      );
      ...
      expect(
        service.isWithinDeliveryRadius(restaurant, farLocation, maxRadiusKm: 30.0),
        isFalse,
      );
    });
  });
