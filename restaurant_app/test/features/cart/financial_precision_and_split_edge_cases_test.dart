...
void main() {
  group('Financial Precision & Currency Rounding Unit Tests', () {
    test('roundCurrency prevents standard floating-point representation errors', () {
    ...
    test('calculateVat computes precise 15% regional tax with correct halala rounding', () {
      // 100 SAR -> 15.00 SAR
    ...
    test('calculatePercentageDiscount clamps accurately and respects max caps', () {
  ...
  group('Bill Splitting & Remainder Halalas Distribution Tests', () {
    test('splitBillDetailed evenly distributes odd remainder halalas (100 SAR / 3 persons)', () {
    ...
    test('splitBillDetailed distributes multiple remainder cents (50 SAR / 7 persons)', () {
    ...
    test('splitBillDetailed handles edge cases: 0 persons, 1 person, negative amount', () {
  ...
  group('CartTotals & CartController Edge Cases Integration', () {
    ...
    test('CartTotals accurately computes subtotal, discount, VAT and total with rounding', () {
      final cartItems = [
    ...
    test('CartController splitTotalDetailed provides exact per-person breakdown', () {
      final controller = CartController();
      controller.addItem(const CartItem(menuItem: item1, quantity: 3)); // 76.50 subtotal
