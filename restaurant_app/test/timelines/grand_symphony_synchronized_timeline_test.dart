...
void main() {
  group('Timeline 8: Grand Symphony Multi-Role Synchronized Journey Test', () {
    ...
    test(
      'Grand Symphony: Manager opens floor -> Customer scans & orders -> Kitchen prepares & prints -> Waiter serves -> Customer pays & rates -> Manager gets real-time analytics',
      () async {
        ...
        cartNotifier.addItem(
          const CartItem(menuItem: lambChops, quantity: 2),
        ); // 95 * 2 = 190.0
        cartNotifier.addItem(
          const CartItem(menuItem: saffronRice, quantity: 2),
        ); // 25 * 2 = 50.0

        // Subtotal = 240.0 SAR
        expect(cartNotifier.totals.subtotal, equals(240.0));

        final discount = symphonyCoupon.calculateDiscount(
          cartNotifier.totals.subtotal,
        );
        ...
        await Future<void>.delayed(const Duration(milliseconds: 10));
        var currentTable = container
            .read(tableControllerProvider)
            .firstWhere((t) => t.id == targetTable.id);
        ...
        expect(
          avgResult.when(onLeft: (_) => 0.0, onRight: (s) => s),
          equals(5.0),
        );
