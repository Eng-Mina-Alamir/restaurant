...
void main() {
  group('Timeline 5: Kitchen KDS Station Full Journey Test', () {
    ...
    test(
      'Kitchen KDS Timeline: Order Inflow -> Alert Sound -> Station Dispatch -> Prep Timer Progression -> Print Ticket -> Mark Ready',
      () async {
        ...
        cartNotifier.addItem(
          const CartItem(menuItem: charcoalChicken, quantity: 2),
        );
        cartNotifier.addItem(
          const CartItem(menuItem: fattoushSalad, quantity: 1),
        );
        ...
        expect(
          kdsOrders.any(
            (o) => o.id == createdOrder!.id && o.status == OrderStatus.pending,
          ),
          isTrue,
        );

        // ── Step 2: Chef acknowledges order and starts preparation ─────────────
        await ordersNotifier.updateStatus(
          createdOrder!.id,
        ...
        final elapsedMinutes = DateTime.now()
            .difference(currentOrder.createdAt)
            .inMinutes;
