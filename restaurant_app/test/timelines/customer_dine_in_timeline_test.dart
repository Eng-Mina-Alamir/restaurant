...
void main() {
  group('Timeline 1: Dine-In Customer Full Journey Test', () {
    ...
    const spicySauce = MenuModifierOption(
      id: 'mod-sauce',
      name: 'صوص حار مميز',
      extraPrice: 3.0,
    );
    ...
    test(
      'Customer Dine-In Timeline: QR Scan -> Browse -> Cart -> Coupon -> Order -> Live Prep -> Serve -> Loyalty & Rating',
      () async {
        ...
        final couponError = couponWelcome.validate(
          cartNotifier.totals.subtotal,
        );
        expect(couponError, isNull);

        final discountAmount = couponWelcome.calculateDiscount(
          cartNotifier.totals.subtotal,
        );
        ...
        var currentOrder = container
            ...
            .firstWhere((o) => o.id == placedOrder.id);
