...
void main() {
  group('Timeline 3: Customer Table Reservation Journey Test', () {
    test(
      'Reservation Timeline: Pick Time & Table -> Confirm Booking -> Table becomes Reserved -> Arrive & Seat -> Release',
      () async {
        ...
        final reservationNotifier = container.read(
        ...
        await tableNotifier.occupy(
          targetTable.id,
          orderId: 'ORD-RES-${targetTable.id}',
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));

        var seatedTable = container
            .read(tableControllerProvider)
            .firstWhere((t) => t.id == targetTable.id);
