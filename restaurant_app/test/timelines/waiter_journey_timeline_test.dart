...
void main() {
  group('Timeline 4: Waiter / Captain Full Journey Test', () {
    ...
    test(
      'Waiter Timeline: Login -> Floor Overview -> Seat Guests -> Take Table Order -> Kitchen Sync -> Serve -> Split Bill -> Clean & Free',
      () async {
        ...
        final table2 = initialTables.firstWhere(
          (t) => t.tableNumber == 2,
          orElse: () => initialTables.first,
        );
        expect(table2.status, equals(TableStatus.available));

        await tableNotifier.occupy(
          table2.id,
          orderId: 'ORD-WAITER-${table2.id}',
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
        var activeTable = container
            .read(tableControllerProvider)
            .firstWhere((t) => t.id == table2.id);
