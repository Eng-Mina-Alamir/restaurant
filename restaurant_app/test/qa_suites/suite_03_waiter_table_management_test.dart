...
void main() {
  group('Suite 3: Waiter & Table Management (تجربة النادل وإدارة الطاولات)', () {
    ...
    // -------------------------------------------------------------
    test('TC-WAIT-01: Table map displays tables and correctly recognizes all TableStatus states', () {
    ...
    test('TC-WAIT-02: Waiter opens order for table, sets items and marks table occupied', () async {
      ...
      final updatedTable = container.read(tableControllerProvider).firstWhere((t) => t.id == 't1');
    ...
    test('TC-WAIT-03: Waiter appends extra items to existing table order', () async {
      ...
      // Initial order
      cartNotifier.addItem(const CartItem(menuItem: sampleItem1, quantity: 1));
      ...
      cartNotifier.addItem(const CartItem(menuItem: sampleItem2, quantity: 2));
      ...
      final allOrders = container.read(ordersControllerProvider).where((o) => o.tableId == 't2').toList();
    ...
    // -------------------------------------------------------------
    test('TC-WAIT-04: Waiter transfers active order from Table A to Table B', () async {
      final container = createQaContainer();
      addTearDown(container.dispose);
      ...
      var t1 = container.read(tableControllerProvider).firstWhere((t) => t.id == 't1');
      ...
      t1 = container.read(tableControllerProvider).firstWhere((t) => t.id == 't1');
      final t3 = container.read(tableControllerProvider).firstWhere((t) => t.id == 't3');
    ...
    // -------------------------------------------------------------
    test('TC-WAIT-05: Settle bill and release table to cleaning state then available', () async {
      final container = createQaContainer();
      addTearDown(container.dispose);
      ...
      var t4 = container.read(tableControllerProvider).firstWhere((t) => t.id == 't4');
      expect(t4.status, TableStatus.occupied);

      // Settle bill -> Table needs cleaning
      await tableNotifier.release('t4', needsCleaning: true);
      t4 = container.read(tableControllerProvider).firstWhere((t) => t.id == 't4');
