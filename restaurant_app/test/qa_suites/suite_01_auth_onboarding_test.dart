...
void main() {
  group('Suite 1: Auth & Onboarding (التثبيت والتهيئة وتسجيل الدخول)', () {
    ...
    // -------------------------------------------------------------
    testWidgets('TC-AUTH-01: Onboarding slides display, swipe, skip and start actions', (
      tester,
    ) async {
      ...
      await tester.tap(find.text('التالي'));
      await tester.pumpAndSettle();
      expect(find.textContaining('دفع متعدد ونقاط ولاء ومكافآت'), findsOneWidget);
    ...
    // -------------------------------------------------------------
    test('TC-AUTH-03: Form validators block invalid emails and short passwords with error messages', () {
      // Email validation
    ...
    // -------------------------------------------------------------
    test('TC-AUTH-04: Multi-role logins assign proper RBAC permissions and default routes', () async {
      final mockAuth = QaMockAuthRemoteDataSource();

      final rolesToTest = [
        {'email': 'customer@restaurant.com', 'expectedRole': UserRole.customer, 'expectedRoute': '/customer'},
        {'email': 'waiter@restaurant.com', 'expectedRole': UserRole.waiter, 'expectedRoute': '/waiter'},
        {'email': 'kitchen@restaurant.com', 'expectedRole': UserRole.kitchen, 'expectedRoute': '/kds'},
        {'email': 'driver@restaurant.com', 'expectedRole': UserRole.driver, 'expectedRoute': '/driver'},
        {'email': 'manager@restaurant.com', 'expectedRole': UserRole.manager, 'expectedRoute': '/manager'},
      ];
      ...
      for (final testCase in rolesToTest) {
        ...
        expect(authState.isAuthenticated, isTrue, reason: 'Failed logging in as $email');
    ...
    // -------------------------------------------------------------
    test('TC-AUTH-05: Authenticated state is maintained and bootstrapped cleanly', () async {
      final mockAuth = QaMockAuthRemoteDataSource();
      final container = createQaContainer(authDataSource: mockAuth);
      addTearDown(container.dispose);
    ...
    // TC-AUTH-06: Logout
    // -------------------------------------------------------------
    test('TC-AUTH-06: Logout clears credentials, active user, and marks unauthenticated', () async {
      final mockAuth = QaMockAuthRemoteDataSource();
      final container = createQaContainer(authDataSource: mockAuth);
      addTearDown(container.dispose);
