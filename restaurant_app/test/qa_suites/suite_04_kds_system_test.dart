...
void main() {
  group('Suite 4: Kitchen Display System - KDS (نظام شاشة المطبخ الذكية)', () {
    ...
    // -------------------------------------------------------------
    test(
      'TC-KDS-01: New placed order immediately appears in KDS active orders stream',
      () async {
        ...
        expect(
          activeKdsOrders.firstWhere((o) => o.id == newOrder!.id).status,
          OrderStatus.pending,
        );
      },
    );
    ...
    // -------------------------------------------------------------
    test(
      'TC-KDS-02: KDS order ticket preserves special dietary instructions and notes',
      () async {
        final container = createQaContainer(
          extraCheckoutItems: [grillItem, drinkItem],
        );
    ...
    // -------------------------------------------------------------
    test(
      'TC-KDS-03: Prep timer calculates elapsed preparation duration accurately',
      () {
        ...
        final elapsedMinutes = DateTime.now()
            .difference(orderPast.createdAt)
            .inMinutes;
    ...
    // -------------------------------------------------------------
    test(
      'TC-KDS-04: KDS moves ticket from Pending -> Preparing -> Ready -> Served',
      () async {
        final container = createQaContainer(
          extraCheckoutItems: [grillItem, drinkItem],
        );
        ...
        expect(
          ordersNotifier.activeOrders.any((o) => o.id == order.id),
          isFalse,
        );
      },
    );
    ...
    // -------------------------------------------------------------
    test(
      'TC-KDS-05: Orders can be filtered by station category (Grill vs Drinks)',
      () async {
        final container = createQaContainer(
          extraCheckoutItems: [grillItem, drinkItem],
        );
        ...
        // Filter Grill
        final grillStationOrders = allOrders
            .where(
        ...
        // Filter Drinks
        final drinkStationOrders = allOrders
            .where(
