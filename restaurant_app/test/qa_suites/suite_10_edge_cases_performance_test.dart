...
void main() {
  ...
  group('Suite 10: Non-Functional & Edge Cases (حالات الشبكة والأداء والأمان)', () {
    ...
    // -------------------------------------------------------------
    test(
      'TC-EDGE-01: Placing an order in offline mode enqueues order safely without crashing',
      () async {
    ...
    // -------------------------------------------------------------
    test(
      'TC-EDGE-02: Each generated order possesses a distinct unique identifier to avoid duplicate charges',
      () async {
    ...
    // -------------------------------------------------------------
    test(
      'TC-EDGE-03: Rapid concurrent placeOrder requests result in single order execution',
      () async {
        final container = createQaContainer(extraCheckoutItems: [testItem]);
        addTearDown(container.dispose);
        await primeMenuForCheckout(container);
    ...
    // -------------------------------------------------------------
    test(
      'TC-EDGE-04: Cart retains items across component rebuilds and state lookups',
      () {
        final container = createQaContainer();
        addTearDown(container.dispose);
    ...
    // -------------------------------------------------------------
    testWidgets(
      'TC-EDGE-05: KDS renders cleanly on Tablet Landscape (1280x800) and Phone (390x844)',
      (tester) async {
