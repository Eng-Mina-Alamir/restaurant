...
void main() {
  ...
  group('Suite 9: Localization & Themes (التعريب والاتجاهات والمظاهر)', () {
    ...
    // -------------------------------------------------------------
    test('TC-LOC-01: Language toggle flips locale and TextDirection between RTL and LTR', () async {
      final container = createQaContainer();
      addTearDown(container.dispose);

      final localeNotifier = container.read(localeControllerProvider.notifier);
      ...
      expect(container.read(currentLanguageProvider).direction, TextDirection.rtl);
      ...
      expect(container.read(currentLanguageProvider).direction, TextDirection.ltr);
    ...
    // -------------------------------------------------------------
    test('TC-LOC-02: AppStrings dictionary contains complete non-empty translations for AR and EN', () {
    ...
    // -------------------------------------------------------------
    test('TC-THM-01: ThemeController switches between Light and Dark mode with valid palettes', () async {
      final container = createQaContainer();
      addTearDown(container.dispose);

      final themeNotifier = container.read(themeModeControllerProvider.notifier);
