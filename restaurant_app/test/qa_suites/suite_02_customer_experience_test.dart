...
void main() {
  group('Suite 2: Customer Experience Flow (تجربة العميل الكاملة)', () {
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-01: Menu browsing, categories and dietary tags filtering',
      () {
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-02: Rapid menu search filters matching items and returns empty for non-existent',
      () {
        final items = MenuSeedData.items;

        // Search in Arabic
        final searchArabic = items
            .where(
              (i) =>
                  i.name.toLowerCase().contains('برجر') ||
                  i.description.toLowerCase().contains('برجر'),
            )
            .toList();
        expect(searchArabic, isNotEmpty);

        // Search by keyword
        final searchMatch = items
            .where(
              (i) =>
                  i.name.contains('بيتزا') || i.description.contains('بيتزا'),
            )
            .toList();
        expect(searchMatch, isNotEmpty);

        // Non-existent search query
        final emptyResult = items
            .where((i) => i.name.contains('صنف_غير_موجود_نهائيا_123'))
            .toList();
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-03: Item customization calculates modifier price additions accurately',
      () {
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-04: Cart item management, tax calculations and grand total accuracy',
      () {
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-05: Coupon application applies discount and rejects expired/inactive codes',
      () {
        // 1. Valid coupon 20%
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-06: Order placement clears active cart and sets order entity',
      () async {
        ...
        final container = createQaContainer(
          extraCheckoutItems: const [testItem],
        );
        addTearDown(container.dispose);
    ...
    // -------------------------------------------------------------
    test(
      'TC-CUST-08: Rating and review submission updates entity and score',
      () async {
        final container = createQaContainer();
        addTearDown(container.dispose);
        ...
        final ratings = await container.read(
          targetRatingsProvider('item-burger-101').future,
        );
