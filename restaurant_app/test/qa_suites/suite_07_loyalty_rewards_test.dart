...
void main() {
  group('Suite 7: Loyalty & Rewards (نظام نقاط الولاء والمكافآت)', () {
    ...
    // -------------------------------------------------------------
    test('TC-LOY-01: Points accrue automatically according to order spend amount', () async {
      ...
      await loyaltyNotifier.earnPoints(orderTotal: 250.0, orderId: 'ORD-LOY-001');
    ...
    // -------------------------------------------------------------
    test('TC-LOY-02: Accumulating threshold points advances tier to Silver and Gold', () {
    ...
    // -------------------------------------------------------------
    test('TC-LOY-03: Customer redeems loyalty reward and points are deducted', () async {
      final container = createQaContainer();
      addTearDown(container.dispose);
      ...
      await loyaltyNotifier.earnPoints(orderTotal: 1000.0, orderId: 'ORD-SEED');
      final beforePoints = container.read(loyaltyControllerProvider).value?.currentPoints ?? 0;
      ...
      final afterPoints = container.read(loyaltyControllerProvider).value?.currentPoints ?? 0;
