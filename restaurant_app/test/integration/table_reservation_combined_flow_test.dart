...
void main() {
  group('Table and Reservation Combined Flow Integration Test', () {
    test('full flow: reservation -> table reserved -> seat customer -> dine-in order -> release table', () async {
      ...
      // 1. Add a table
      await tableController.addTable(
        tableNumber: 10,
        capacity: 4,
      );
      ...
      final reservationCreated = await reservationController.createReservation(
      ...
      expect(tablesAfterRes.firstWhere((t) => t.id == table.id).status, TableStatus.reserved);
      ...
      expect(tablesAfterSeat.firstWhere((t) => t.id == table.id).status, TableStatus.occupied);
      ...
      expect(tablesAfterDone.firstWhere((t) => t.id == table.id).status, TableStatus.available);
    });
  });
}
