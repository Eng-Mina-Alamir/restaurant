...
void main() {
  group('Realtime Sync & Broadcast Integration Flow', () {
    test('broadcasts order and driver events through real-time event bus stream',
        () async {
      ...
      final expectation = expectLater(
        ...
        emitsInOrder(<Matcher>[
          isA<RealtimeEvent>()
              ...
              .having((e) => e.payload['orderId'], 'payload.orderId', 'ORD-RT-1')
              .having((e) => e.payload['status'], 'payload.status', 'pending'),
          isA<RealtimeEvent>()
              .having((e) => e.type, 'type',
                  RealtimeEventType.driverLocationUpdated)
              .having(
                  (e) => e.payload['driverId'], 'payload.driverId', 'DRV-7')
              .having((e) => e.payload['latitude'], 'payload.latitude', 30.05)
              .having(
                  (e) => e.payload['longitude'], 'payload.longitude', 31.24)
              .having(
                  (e) => e.payload['orderId'], 'payload.orderId', 'ORD-RT-1'),
          isA<RealtimeEvent>()
              .having(
                  (e) => e.type, 'type', RealtimeEventType.tableStatusChanged)
              .having((e) => e.payload['tableId'], 'payload.tableId', 'TBL-4')
              .having((e) => e.payload['status'], 'payload.status', 'occupied'),
        ]),
      );

      realtime.broadcastOrderCreated(
        const {'orderId': 'ORD-RT-1', 'status': 'pending'},
      );
      ...
      realtime.broadcastTableStatusChanged(
        const {'tableId': 'TBL-4', 'status': 'occupied'},
      );
