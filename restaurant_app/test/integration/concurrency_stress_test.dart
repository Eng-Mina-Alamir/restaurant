...
void main() {
  group('Concurrency & High-Load Stress Integration Tests', () {
    ...
    test(
      'Concurrent cart item modifications maintain calculation integrity',
      () async {
        ...
        final item1InCart = state.firstWhere(
          (i) => i.menuItem.id == sampleItem1.id,
        );
        final item2InCart = state.firstWhere(
          (i) => i.menuItem.id == sampleItem2.id,
        );

        expect(item1InCart.quantity, equals(25));
        expect(item2InCart.quantity, equals(50));
        expect(
          cartController.totals.subtotal,
          equals((25 * 60.0) + (50 * 20.0)),
        );
      },
    );

    test(
      'Multi-table concurrent status transitions in TableController',
      () async {
        ...
        if (tables.isNotEmpty) {
          ...
          for (final tbl in tables) {
            futures.add(
              tableController.occupy(tbl.id, orderId: 'ORD-${tbl.id}'),
            );
          }
          await Future.wait(futures);
    ...
    test(
      'Rapid order placement and status transitions through ProviderContainer',
      () async {
